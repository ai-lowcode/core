...
import { AlButton, AlCheckbox, AlInput } from '@ai-lowcode/element-plus'
...
<template>
  <div class="flex justify-center items-center h-full" :style="`background: #fff url(${bg})`">
    <div class="w-[900px] h-[480px] bg-white rounded-md flex" style="box-shadow: 0 1px 14px 2px rgb(7 27 48 / 9%)">
      <img class="w-[420px] h-full" :src="loginBg" alt="登录背景">
      <div class="p-[60px] flex-1">
        <div class="text-xl text-center">
          欢迎登录 Ai Lowcode
        </div>
        <div class="mt-[40px]">
          <AlInput v-model="loginForm.username" prefix-icon="" placeholder="请输入账户" />
          <AlInput v-model="loginForm.password" class="mt-[20px]" prefix-icon="" placeholder="请输入密码" />
          <div class="flex justify-between items-center mt-[15px]">
            <AlCheckbox>记住密码</AlCheckbox>
            <div class="text-sm cursor-pointer text-blue-600">
              忘记密码
            </div>
          </div>
          <div class="mt-[20px]">
            <AlButton class="w-full" type="primary">
              登录
            </AlButton>
          </div>
          <div class="mt-[20px]">
