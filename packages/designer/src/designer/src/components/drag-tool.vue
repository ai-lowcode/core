...
<template>
  <div class="outline relative pb-2 duration-300" :class="selectComponentCtx.selectComponent.value === id ? 'outline-2 outline-blue-600' : 'outline-1 hover:outline-dashed hover:outline-blue-600 outline-transparent'" @click.stop="active">
    <div :class="!hiddenBtn && selectComponentCtx.selectComponent.value === id && dragBtn ? 'al-drag-btn cursor-move' : ''">
      <div v-if="!hiddenBtn" class="absolute top-0 left-0">
        <div v-if="selectComponentCtx.selectComponent.value === id && dragBtn" class="w-[18px] h-[18px] text-white bg-blue-600" style="cursor: move;">
