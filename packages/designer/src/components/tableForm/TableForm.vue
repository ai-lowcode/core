...
export default {
  ...
  methods: {
    ...
    updateRaw(tr) {
      ...
      tr.children[0].props.textContent = idx + 1
...
<template>
  <div class="_fc-table-form" :class="{ '_fc-disabled': disabled }">
    <component
      :is="Form" v-model:api="fapi" :option="options" :rule="rule"
      :extend-option="true"
      :disabled="disabled"
      @change="formChange"
      @emit-event="$emit"
    />
    <el-button
      v-if="!max || max > trs.length" link type="primary" class="fc-clock"
      @click="addRaw(true)"
    >
      <i class="fc-icon icon-add-circle" style="font-weight: 700;" />
      添加
    </el-button>
  </div>
</template>
