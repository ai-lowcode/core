...
import { AlButton, AlCheckbox, AlIcon, AlInput, Hide, Lock, SuccessFilled, User, View } from '@ai-lowcode/element-plus'
...
async function handleVerificationCode() {
  const { data, code } = await AlHttp.get('https://nest-api.buqiyuan.site/api/auth/captcha/img', {
    width: 100,
    height: 50,
  }, {
...
<template>
  <div class="flex justify-center items-center h-full" :style="`background: #fff url(${bg})`">
    <div class="w-[900px] h-[480px] bg-white rounded-md flex" style="box-shadow: 0 1px 14px 2px rgb(7 27 48 / 9%)">
      ...
      <div class="p-[60px] flex-1">
        ...
        <div class="mt-[40px]">
          <AlInput v-model="loginForm.username" :prefix-icon="User" placeholder="请输入账户" />
          <AlInput v-model="loginForm.password" :prefix-icon="Lock" :type="showPassword ? 'text' : 'password'" class="mt-[20px]" placeholder="请输入密码">
            <template #suffix>
          ...
          <div class="mt-[20px]">
            <AlButton class="w-full" type="primary" @click="handleLogin">
              登录
            </AlButton>
          </div>
          <div class="mt-[15px]">
